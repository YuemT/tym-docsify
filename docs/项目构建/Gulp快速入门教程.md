...
4. 使用gulp插件
  ...
  * 自动编译
    ...
    * 热加载(实时加载)
        * 下载插件：gulp-connect
        ```
        1、 npm install gulp-connect --save-dev//创建服务器插件
            npm install open --save-dev  //自动开启链接插件
        2、 注册 热加载的任务 server，注意依赖default任务
        2、注册热加载的任务
            //配置加载的选项
            connect.server({
            ...
            * .pipe(connect.reload());//注意在每一项需要实时刷新的任务里配置此选项
        ```
    ...
    * 扩展
        ...
        * 使用方法：
            ...
            * 所有的插件用 $ 引出，其他插件的方法名统一为插件的功能名字(即插件名字的最后一部分)：如：concat,connect,cssmin...
            ...
            ```
* Gulp 配置文件示例:[gulpfile.js](./gulpfile.js)